# Standalone correlation scatter template
...
figure_spec <- pp_figure_spec(
  figure_id = figure_id,
  template_id = "correlation-scatter-template",
  scientific_message = "Assess an x-y association with transparent trend context.",
...
palette_check <- if (!is.null(group_col)) pp_validate_palette(df[[group_col]], "discrete") else pp_qa_result("palette", "pass", "single trend color")
layout <- pp_estimate_canvas_size(1, preset = preset)
layout_check <- pp_assess_layout_risk(1, plot_type = "association", label_strategy = label_strategy)

mapping <- aes(x = .data[[x_col]], y = .data[[y_col]])
if (!is.null(group_col)) mapping <- aes(x = .data[[x_col]], y = .data[[y_col]], colour = .data[[group_col]])

p <- ggplot(df, mapping) +
  geom_point(size = pp_point_size("normal"), alpha = 0.78) +
  geom_smooth(aes(group = 1), method = "lm", se = TRUE, linewidth = pp_line_width("interval"), colour = "#4D4D4D", fill = "#BDBDBD", alpha = 0.18) +
...
pp_write_notes(notes_path, figure_id, input_path, output_files, preset,
  design_decisions = c("pattern: scatter-regression-marginal", "point cloud is primary evidence", "single global linear trend is secondary context", "gridlines disabled", "correlation statistics can be added explicitly"),
  qa_checks = paste(qa_results$gate, qa_results$status, qa_results$note, sep = ": "),
  remaining_issues = "Add reported correlation statistics if required by the manuscript",
...
pp_write_metadata(metadata_path, figure_spec, metric_spec, output_files, layout = layout,
  palette = list(type = if (!is.null(group_col)) "discrete" else "none", name = "graphpad_discrete"), ordering = list(rule = "input order"),
