...
if (any(!(!is.na(df[[row_col]]) & !is.na(df[[column_col]]) & !is.na(df[[value_col]])))) stop("Incomplete required fields; explicit upstream missing-value handling is required.")
...
outputs <- pp_save_all_with_qa_loop(plot, output_stem, render_spec = pp_render_spec(n_panels = pp_infer_panel_count(plot), panel_tags = inherits(plot, "patchwork"), width_mm = (8.9) * 10, height_mm = (7.2) * 10), preset = figure_spec$output_preset, qa_context = list(family = figure_spec$plot_type), overwrite = FALSE)
