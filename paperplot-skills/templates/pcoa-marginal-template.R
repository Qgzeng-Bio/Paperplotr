...
df[[pc1_col]] <- pp_numeric_field(df[[pc1_col]], pc1_col); df[[pc2_col]] <- pp_numeric_field(df[[pc2_col]], pc2_col)
if (any(!(!is.na(df[[pc1_col]]) & !is.na(df[[pc2_col]]) & !is.na(df[[group_col]])))) stop("Incomplete required fields; explicit upstream missing-value handling is required.")
...
outputs <- pp_save_all_with_qa_loop(plot, output_stem, render_spec = pp_render_spec(n_panels = pp_infer_panel_count(plot), panel_tags = inherits(plot, "patchwork"), width_mm = (8.9) * 10, height_mm = (6.2) * 10), preset = figure_spec$output_preset, qa_context = list(family = figure_spec$plot_type), overwrite = FALSE)
