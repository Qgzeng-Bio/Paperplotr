...
if (any(!(!is.na(df[[sample_col]]) & !is.na(df[[mode_col]]) & !is.na(df[[group_col]]) & !is.na(df[[value_col]])))) stop("Incomplete required fields; explicit upstream missing-value handling is required.")
mode_levels <- unique(df[[mode_col]])
group_levels <- unique(df[[group_col]])
if(any(df[[value_col]]<0) || anyDuplicated(df[c(sample_col,mode_col)])) stop('Duplication burdens require nonnegative values and one row per sample/mode.')
if(any(vapply(split(df[[group_col]],df[[sample_col]]),function(x) length(unique(x))!=1,logical(1)))) stop('Each sample must belong to one group.')
...
frac <- df
frac$value <- frac[[value_col]]
frac$panel <- "B. Mode-specific burden"
...
if(any(sample_total<=0)) stop('Per-sample composition denominators must be positive.')
rel <- df
...
plot_df$panel <- factor(plot_df$panel, levels = c("A. Duplicate-pair burden", "B. Mode-specific burden", "C. Relative contribution", "D. Group effect size"))
