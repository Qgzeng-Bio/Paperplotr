...
if (any(!(!is.na(df[[category_col]]) & !is.na(df[[value_col]])))) stop("Incomplete required fields; explicit upstream missing-value handling is required.")
...
label_strategy <- pp_label_strategy_v2(unique(recipe_df$category), figure_role = figure_role, available_width_cm = 8.9, sample_identity_role = "core", axis = "y", available_height_cm = 5.0)
...
plot <- pp_apply_rank_labels(plot, as.character(recipe_df$category), label_strategy, label_key_path)
