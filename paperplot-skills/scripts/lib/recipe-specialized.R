...
pp_recipe_specialized <- function(entry,d,params,spec) {
  switch(entry$handler,
    complex_heatmap = {
      ...
      if(clustering && (is.null(params$distance) || is.null(params$linkage))) stop('Declare distance and linkage for heatmap clustering, or provide an upstream dendrogram.')
      ...
      heat <- ComplexHeatmap::Heatmap(mat,name=params$value_label %||% 'Value',
        ...
        top_annotation=if(!is.null(annotations)&&axis=='column') ComplexHeatmap::HeatmapAnnotation(df=annotations[cols,,drop=FALSE],annotation_name_gp=grid::gpar(fontfamily=spec$family,fontsize=spec$text_pt$legend)) else NULL,
        left_annotation=if(!is.null(annotations)&&axis=='row') ComplexHeatmap::rowAnnotation(df=annotations[rows,,drop=FALSE],annotation_name_gp=grid::gpar(fontfamily=spec$family,fontsize=spec$text_pt$legend)) else NULL,
    ...
    network = {
      ...
      if(is.null(params$directed)) stop('Declare network directed=TRUE/FALSE; arrow meaning cannot be guessed.')
      ...
      ggraph::ggraph(graph,layout=params$layout %||% 'fr')+
        ...
        ggraph::geom_node_text(ggplot2::aes(label=name),repel=TRUE,family=spec$family,size=spec$text_pt$annotation/ggplot2::.pt)+
    ...
    flow = {
      ...
      if(variant=='sankey') {
        ...
        ggplot2::ggplot(edges,ggplot2::aes(axis1=source,axis2=target,y=weight))+
          ggalluvial::geom_alluvium(ggplot2::aes(fill=source),alpha=.65,width=.18)+
          ggalluvial::geom_stratum(width=.18,fill='#DDDDDD')+
          ggplot2::geom_text(stat='stratum',ggplot2::aes(label=ggplot2::after_stat(stratum)),size=pp_text_size('minimum'))+
          ggplot2::scale_x_discrete(limits=c('Source','Target'),expand=c(.1,.1))+pp_theme()
      } else {
        ...
        pp_capture_vector(function() gridGraphics::grid.echo(function() {
          old <- graphics::par(family=spec$family,ps=spec$text_pt$body,mar=c(1,1,1,1)); on.exit(graphics::par(old))
          circlize::circos.clear(); on.exit(circlize::circos.clear(),add=TRUE)
          circlize::chordDiagram(edges,directional=as.integer(isTRUE(params$directed)),annotationTrack='grid',
          ...
          circlize::circos.trackPlotRegion(track.index=1,panel.fun=function(x,y) {
            ...
            circlize::circos.text(mean(xl),.5,sector,cex=spec$text_pt$annotation/spec$text_pt$body,facing='clockwise',niceFacing=TRUE)
          },bg.border=NA)
        },newpage=FALSE),spec)
      }
    },
    circos = {
      ...
      pp_capture_vector(function() gridGraphics::grid.echo(function() {
        ...
        for(track in tracks) {
          ...
          circlize::circos.trackPlotRegion(factors=rows$chr,x=(rows$start+rows$end)/2,y=rows$value,
            panel.fun=function(x,y) circlize::circos.points(x,y,pch=16,cex=.35,col='#4E79A7'))
        }
      },newpage=FALSE),spec)
    },
    spatial = {
      ...
      if(entry$variant=='polygon') {
        geometry <- if(inherits(d,'sf')) d else params$geometry
        ...
        ggplot2::ggplot(shape)+ggplot2::geom_sf(ggplot2::aes(fill=value),linewidth=.15)+ggplot2::scale_fill_gradientn(colours=pp_gradient_palette())+pp_theme()
      } else {
        ...
        ggplot2::ggplot(shape)+ggplot2::geom_sf(ggplot2::aes(colour=value),size=pp_point_size('normal'))+
          ggplot2::scale_colour_gradientn(colours=pp_gradient_palette())+pp_theme()
      }
    },
    tree = {
      ...
      if(entry$variant%in%c('annotation','circular')) {
        ...
        q <- ggtree::gheatmap(q,values,width=.2,colnames=TRUE,font.size=spec$text_pt$legend/ggplot2::.pt)
      }
      q
    },
